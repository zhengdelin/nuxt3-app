...
// 註冊攔截器
export function registerAxiosInterceptors($axios: AxiosInstance, _: NuxtApp) {
  // console.log("app :>> ", app);
  ...
  $axios.interceptors.response.use(
    (response) => {
      ...
      if (process.env.NODE_ENV !== "production" && process.client) console.log(`${config.method} ${config.url}: response->`, response);
    ...
    (error: AxiosError<any>) => {
      ...
      if (errorResponse) {
        ...
        if (process.env.NODE_ENV !== "production" && process.client)
          console.error(`${config.method} ${errorResponse.config.url}: response->`, errorResponse);
...
export function useAxios(baseURL: string, app: NuxtApp) {
  ...
  // 註冊攔截器
  registerAxiosInterceptors($axios, app);
